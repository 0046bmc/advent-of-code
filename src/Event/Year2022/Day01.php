<?php

declare(strict_types=1);

namespace App\Event\Year2022;

use mahlstrom\AoC\DayBase;
use mahlstrom\AoC\DayInterface;

class Day01 extends DayBase implements DayInterface
{
    public function testPart1(): iterable
    {
        yield '24000' => file_get_contents(__DIR__ . '/TestInputs/Day01.1.txt');
    }

    public function testPart2(): iterable
    {
        yield '45000' => file_get_contents(__DIR__ . '/TestInputs/Day01.1.txt');
    }

    public function solvePart1(string $input): string
    {
//        $in = explode("\n\n", chop($input));
//        $elfs = [];
//        foreach ($in as $item) {
//            $cals = explode("\n", chop($item));
//            $elfs[] = array_sum($cals);
//        }
//        sort($elfs);
        $elfs = $this->getCaloriesPerElf($input);
        return (string)array_pop($elfs);
    }

    public function solvePart2(string $input): string
    {
        $elfs = $this->getCaloriesPerElf($input);
        $tot = (int)array_pop($elfs);
        $tot += (int)array_pop($elfs);
        $tot += (int)array_pop($elfs);

        return (string)$tot;
    }

    private function getCaloriesPerElf($input)
    {
        $in = explode("\n\n", chop($input));
        $elfs = [];
        foreach ($in as $item) {
            $cals = explode("\n", chop($item));
            $elfs[] = array_sum($cals);
        }
        sort($elfs);
        return $elfs;
    }
}
